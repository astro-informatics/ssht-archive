...
void ssht_test_gen_flm_complex(_Complex double *flm, int L, int spin, int seed);
void ssht_test_gen_flm_real(_Complex double *flm, int L, int seed);
void ssht_test_gen_lb_flm_complex(_Complex double *flm, int L0, int L, int spin, int seed);
void ssht_test_gen_lb_flm_real(_Complex double *flm, int L0, int L, int seed);

int main(int argc, char *argv[]) {

  _Complex double *flm_orig, *flm_syn;
  _Complex double *f_mw, *f_mw_ss, *f_gl, *f_dh;
  double *f_mw_real, *f_mw_ss_real, *f_gl_real, *f_dh_real;
  _Complex double *f_mw_lb, *f_mw_lb_ss;
  double *f_mw_lb_real, *f_mw_lb_ss_real;

  _Complex double *f_mw_pole, *f_mw_ss_pole;
  _Complex double f_mw_sp, f_mw_ss_np, f_mw_ss_sp;
  ...
  // Allocate memory.
  flm_orig = (_Complex double*)calloc(L*L, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(flm_orig)
  flm_syn = (_Complex double*)calloc(L*L, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(flm_syn)
  f_mw = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_mw)
  f_mw_lb = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_mw)
  f_mw_ss = (_Complex double*)calloc((L+1)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_mw_ss)
  f_mw_lb_ss = (_Complex double*)calloc((L+1)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_mw_ss)
  f_mw_pole = (_Complex double*)calloc((L-1)*(2*L-1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_mw_pole)
  f_mw_ss_pole = (_Complex double*)calloc((L-1)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_mw_ss_pole)
  f_gl = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_gl)
  f_dh = (_Complex double*)calloc((2*L)*(2*L-1), sizeof(_Complex double));
