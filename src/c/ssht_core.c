...
void ssht_core_mw_lb_inverse_sov_sym(_Complex double *f, const _Complex double *flm,
  ...
  _Complex double mmfactor;
  double *dl;
  double *dl8 = NULL;
  int dl_offset, dl_stride;
  _Complex double *exps, *m_factors;
  int exps_offset;
  double elmmsign, elssign;
  int spinneg;
  _Complex double *Fmm, *fext;
  ...
  exps = (_Complex double*)calloc(2*L-1, sizeof(_Complex double));
  ...
  // Compute Fmm.
  Fmm = (_Complex double*)calloc((2*L-1)*(2*L-1), sizeof(_Complex double));
  ...
  fext = (_Complex double*)calloc((2*L-1)*(2*L-1), sizeof(_Complex double));
  ...
  memcpy(f, fext, L*(2*L-1)*sizeof(_Complex double));
...
void ssht_core_mw_lb_inverse_sov_sym_real(double *f, const _Complex double *flm,
  ...
  int el2pel, m_offset;
  double ssign, elfactor;
  _Complex double *m_factors;
  _Complex double mmfactor;
  double *dl;
  double *dl8 = NULL;
  int dl_offset, dl_stride;
  _Complex double *exps;
  int exps_offset;
  double elmmsign, elssign;
  int spinneg;
  _Complex double *Fmm, *Fmm_shift;
  double *fext_real;
  int Fmm_offset, Fmm_stride, fext_stride;
  fftw_plan plan;
  int spin = 0;
  ...
  // Compute Fmm.
  Fmm = (_Complex double*)calloc((2*L-1)*L, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmm)
  Fmm_offset = L-1;
  Fmm_stride = L;
  dl = ssht_dl_calloc(L, SSHT_DL_QUARTER);
  ...
  // Apply spatial shift.
  Fmm_shift = (_Complex double*)calloc((2*L-1)*L, sizeof(_Complex double));
...
void ssht_core_mwdirect_inverse_sov(_Complex double *f, const _Complex double *flm,
  ...
  // Compute ftm.
  ftm = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  ...
  // Compute f.
  inout = (_Complex double*)calloc(2*L-1, sizeof(_Complex double));
...
void ssht_core_mw_lb_forward_sov_conv_sym(_Complex double *flm, const _Complex double *f,
  ...
  _Complex double *inout;
  _Complex double *Fmt, *Fmm, *Gmm, *m_mm_factor;
  _Complex double *w, *wr;
  _Complex double *Fmm_pad, *tmp_pad;
  int f_stride, Fmt_stride, Fmt_offset, Fmm_stride, Fmm_offset;
  double *dl;
  double *dl8 = NULL;
  int dl_offset, dl_stride;
  int w_offset;
  _Complex double *expsm, *expsmm;
  int exps_offset;
  ...
  signs = (double*)calloc(L+1, sizeof(double));
  SSHT_ERROR_MEM_ALLOC_CHECK(signs)
  expsm = (_Complex double*)calloc(2*L-1, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(expsm)
  expsmm = (_Complex double*)calloc(2*L-1, sizeof(_Complex double));
  ...
  Fmt = (_Complex double*)calloc((2*L-1)*(2*L-1), sizeof(_Complex double));
  ...
  Fmm_offset = L-1;
  for (m=-(L-1); m<=L-1; m++) {
    memcpy(inout, &Fmt[(m+Fmt_offset)*Fmt_stride], Fmt_stride*sizeof(_Complex double));
    fftw_execute_dft(plan, inout, inout);
    for(mm=0; mm<=L-1; mm++)
  ...
  // Compute weights.
  w = (double complex*)calloc(4*L-3, sizeof(_Complex double));
  ...
  wr = (double complex*)calloc(4*L-3, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(wr)
  inout = (_Complex double*)calloc(4*L-3, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(inout)
  ...
  Fmm_pad = (_Complex double*)calloc(4*L-3, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmm_pad)
  tmp_pad = (_Complex double*)calloc(4*L-3, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(tmp_pad)
  Gmm = (_Complex double*)calloc((2*L-1)*(2*L-1), sizeof(_Complex double));
...
void ssht_core_mw_lb_forward_sov_conv_sym_real(_Complex double *flm, const double *f,
  ...
  signs = (double*)calloc(L+1, sizeof(double));
  SSHT_ERROR_MEM_ALLOC_CHECK(signs)
  expsm = (_Complex double*)calloc(L, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(expsm)
  expsmm = (_Complex double*)calloc(2*L-1, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(expsmm)
  ...
  // Compute Fourier transform over phi, i.e. compute Fmt.
  Fmt = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmt)
  Fmt_stride = 2*L-1;
  Fmt_offset = L-1;
  f_stride = 2*L-1;
  ...
  out = (_Complex double*)calloc(L, sizeof(_Complex double));
  ...
  // Compute Fourier transform over theta, i.e. compute Fmm.
  Fmm = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmm)
  Fmm_stride = 2*L-1;
  Fmm_offset = L-1;
  inout = (_Complex double*)calloc(2*L-1, sizeof(_Complex double));
  ...
  for (m=0; m<=L-1; m++) {
    memcpy(inout, &Fmt[m*Fmt_stride], Fmt_stride*sizeof(_Complex double));
    fftw_execute_dft(plan, inout, inout);
    for(mm=0; mm<=L-1; mm++)
  ...
  tmp_pad = (_Complex double*)calloc(4*L-3, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(tmp_pad)
  Gmm = (_Complex double*)calloc((2*L-1)*L, sizeof(_Complex double));
  Gmm_stride = L;
  SSHT_ERROR_MEM_ALLOC_CHECK(Gmm)
  for (m=0; m<=L-1; m++) {
...
void ssht_core_mw_inverse_sov_sym_pole(_Complex double *f,
  ...
  _Complex double* f_full;
  int f_stride = 2*L-1;

  // Allocate full array.
  f_full = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  ...
  memcpy(f, f_full, (L-1)*(2*L-1)*sizeof(_Complex double));
...
void ssht_core_mw_forward_sov_conv_sym_pole(_Complex double *flm, const _Complex double *f,
  ...
  f_full = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_full)
  memcpy(f_full, f, (L-1)*(2*L-1)*sizeof(_Complex double));
...
void ssht_core_mw_lb_inverse_sov_sym_ss(_Complex double *f, const _Complex double *flm,
  ...
  Fmm = (_Complex double*)calloc((2*L)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmm)
  Fmm_offset = L-1;
  Fmm_stride = 2*L;
  dl = ssht_dl_calloc(L, SSHT_DL_QUARTER);
  ...
  fext = (_Complex double*)calloc((2*L)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(fext)
  fext_stride = 2*L;
  ...
  // Note that t and p indices of fext are increased in size by
  // one compared to usual sampling.
  memcpy(f, fext, (L+1)*(2*L)*sizeof(_Complex double));
...
void ssht_core_mw_lb_inverse_sov_sym_ss_real(double *f, const _Complex double *flm,
  ...
  // Note that m and mm indices are increased in size by one and
  // will be filled with zeros by calloc.
  Fmm = (_Complex double*)calloc((2*L)*(L+1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmm)
  Fmm_offset = L-1;
  Fmm_stride = L+1;
  dl = ssht_dl_calloc(L, SSHT_DL_QUARTER);
  ...
  // Apply spatial shift.
  Fmm_shift = (_Complex double*)calloc((2*L)*(L+1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmm_shift)
  for (mm=0; mm<=L-1; mm++)
    for (m=0; m<=L-1; m++)
      Fmm_shift[mm*Fmm_stride + m] =
...
void ssht_core_mw_lb_forward_sov_conv_sym_ss(_Complex double *flm, const _Complex double *f,
  ...
  // Note that t and p indices of fext are increased in size by
  // one compared to usual sampling.
  Fmt = (_Complex double*)calloc((2*L)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmt)
  Fmt_stride = 2*L;
  Fmt_offset = L-1;
  f_stride = 2*L;
  inout = (_Complex double*)calloc(2*L, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(inout)
...
void ssht_core_mw_lb_forward_sov_conv_sym_ss_real(_Complex double *flm, const double *f,
  ...
  // Note that t and p indices of fext are increased in size by
  // one compared to usual sampling.
  Fmt = (_Complex double*)calloc((L+1)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmt)
  Fmt_stride = 2*L;
  Fmt_offset = L-1;
  f_stride = 2*L;
  ...
  SSHT_ERROR_MEM_ALLOC_CHECK(in_real)
  out = (_Complex double*)calloc(L+1, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(out)
  ...
  // Compute Fourier transform over theta, i.e. compute Fmm.
  // Note that m and mm indices are increased in size by one.
  Fmm = (_Complex double*)calloc((L+1)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Fmm)
  Fmm_stride = 2*L;
  Fmm_offset = L-1;
  inout = (_Complex double*)calloc(2*L, sizeof(_Complex double));
...
void ssht_core_mw_inverse_sov_sym_ss_pole(_Complex double *f,
  ...
  // Allocate full array.
  f_full = (_Complex double*)calloc((L+1)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(f_full)
...
void ssht_core_gl_inverse_sov_real(double *f, const _Complex double *flm,
  ...
  // Compute ftm.
  ftm = (_Complex double*)calloc(L*L, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(ftm)
  ftm_stride = L;
  ftm_offset = 0;
  ...
  // Compute f.
  in = (_Complex double*)calloc(L, sizeof(_Complex double));
  ...
  f_stride = 2*L-1;
  for (t=0; t<=L-1; t++) {
    memcpy(in, &ftm[t*ftm_stride], L*sizeof(_Complex double));
...
void ssht_core_dh_inverse_sov(_Complex double *f, const _Complex double *flm,
  ...
  // Compute ftm.
  ftm = (_Complex double*)calloc((2*L)*(2*L-1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(ftm)
  ftm_stride = 2*L-1;
  ftm_offset = L-1;
  dlm1p1_line = (double*)calloc(2*L-1, sizeof(double));
...
void ssht_core_dh_inverse_sov_real(double *f, const _Complex double *flm,
  ...
  // Compute ftm.
  ftm = (_Complex double*)calloc(2*L*L, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(ftm)
  ftm_stride = L;
  ftm_offset = 0;
  dlm1p1_line = (double*)calloc(L, sizeof(double));
