...
void ssht_core_mw_lb_inverse_sov_sym(complex double *f, const complex double *flm,
  ...
  if (ssht_use_libsharp_inv())
    ...
    // run the SHT proper
    sharp_execute(SHARP_ALM2MAP,abs(spin),alm,&frp[0],tinfo,alms,(spin==0)?2:1,SHARP_DP,NULL,NULL);
    if (spin<0)
      for (int i=0; i<L*(2*L-1); ++i)
        f[i]=conj(f[i]);
    //cleanup
    DEALLOC2D(alm);
...
void ssht_core_gl_forward_sov(complex double *flm, const complex double *f,
  ...
  if (ssht_use_libsharp_fwd())
    ...
    if (spin>=0)
      ...
      {
      fr=RALLOC(double,2*L*(2*L-1));
      for (int i=0; i<L*(2*L-1); ++i)
        { fr[2*i]=creal(f[i]); fr[2*i+1] = -cimag(f[i]); }
      }
    double *frp[2];
    frp[0]=fr;
    frp[1]=fr+1;
