 ...
 * \author <a href="http://www.jasonmcewen.org">Jason McEwen</a>
 */
void ssht_adjoint_mw_forward_sov_sym(_Complex double *f, _Complex double *flm,
				     int L, int spin,
				     ssht_dl_method_t dl_method,
				     int verbosity) {
  ...
  Ftm = (_Complex double*)calloc((2*L-1)*(2*L-1), sizeof(_Complex double));
...
void ssht_adjoint_mw_forward_sov_sym_real(double *f, 
  ...
  tmp_pad = (_Complex double*)calloc(4*L-3, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(tmp_pad)
  Gmm = (_Complex double*)calloc(L*(2*L-1), sizeof(_Complex double));
  Gmm_stride = 2*L-1;
  SSHT_ERROR_MEM_ALLOC_CHECK(Gmm)
  for (m=0; m<=L-1; m++) {
  ...
  SSHT_ERROR_MEM_ALLOC_CHECK(inout)
  plan = fftw_plan_dft_1d(2*L-1, inout, inout, FFTW_BACKWARD, FFTW_MEASURE);
  Ftm = (_Complex double*)calloc((2*L-1)*L, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Ftm)
  Ftm_stride = L;
  for (m=0; m<=L-1; m++) {
  ...
  for(t=0; t<=L-1; t++) {
    ...
    memcpy(in, &Ftm[t*Ftm_stride], Ftm_stride*sizeof(_Complex double));
...
void ssht_adjoint_mw_forward_sov_sym_ss(_Complex double *f, _Complex double *flm,
  ...
  tmp_pad = (_Complex double*)calloc(4*L-3, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(tmp_pad)
  Gmm = (_Complex double*)calloc((2*L)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Gmm)
  Gmm_stride = 2*L;
  Gmm_offset = L-1;
  for (m=-(L-1); m<=L-1; m++) {
  ...
  Ftm = (_Complex double*)calloc((2*L)*(2*L), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Ftm)
  Ftm_stride = 2*L;
  Ftm_offset = L-1;
  for (m=-(L-1); m<=L; m++) {
...
void ssht_adjoint_mw_forward_sov_sym_ss_real(double *f, 
  ...
  tmp_pad = (_Complex double*)calloc(4*L-3, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(tmp_pad)
  Gmm = (_Complex double*)calloc((L+1)*(2*L), sizeof(_Complex double));
  Gmm_stride = 2*L;
  Gmm_offset = L-1;
  SSHT_ERROR_MEM_ALLOC_CHECK(Gmm)
  for (m=0; m<=L-1; m++) {
  ...
  SSHT_ERROR_MEM_ALLOC_CHECK(inout)
  plan = fftw_plan_dft_1d(2*L, inout, inout, FFTW_BACKWARD, FFTW_MEASURE);
  Ftm = (_Complex double*)calloc((2*L)*(L+1), sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(Ftm)
  Ftm_stride = L+1;
  for (m=0; m<=L; m++) {
  ...
  SSHT_ERROR_MEM_ALLOC_CHECK(out_real)
  in = (_Complex double*)calloc(L+1, sizeof(_Complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(in)
